...
def withinSask(x, coord):
    if coord == "longitude":
        ...
        print(
            "Please specify if this split test is for longitude or latitude; parameter missing"
        )
...
long_split = float(
    raw_input(
        "Please specify the longitudinal split. This determines quadrant extent with respect to the X axis"
    )
)
while long_split < West or long_split > East:
    long_split = float(
        raw_input("Please try again; stay between {0} and {1}".format(West, East))
    )

lat_split = float(
    raw_input(
        "Please specify the latitudinal split. This determines quadrant extent with respect to the Y axis"
    )
)
while lat_split > North or lat_split < South:
    lat_split = float(
        raw_input("Please try again; stay between {0} and {1}".format(North, South))
    )
...
for i in range(1, n_sites):
    rand_lat = random.uniform(
        South, North
    )  # We want to generate random points that fall beteen Northern most and Southern most extent of Saskatchewan
    rand_long = random.uniform(
        East, West
    )  # We want to generate random points that fall between the Eastern most and Western most extent of Saskatchewan
