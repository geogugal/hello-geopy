...
print(
    "Welcome to the Wikipedia City Information - currently in Beta"
)  # WELCOME MESSAGE
city_list = [
    "Montreal",
    "Singapore",
...
city = (
    raw_input("Please select one of the above cities to explore").lower().capitalize()
)  # RAW INPUT FROM USER; SELECT CITY # .lower() & capitalize() USED TO REMOVE K SENSITIVITY
...
city_dict = {
    "Montreal": {
        "population": 1780000,
        "category": "reside in",
        "fact": "Montreal is verifiably the best city on Earth.",
        "coordinates": (45.50, -73.56),
    },
    "Singapore": {
        "population": 5612000,
        "category": "want to visit",
        "fact": "Singapore is a major global transport and trade hub.",
        "coordinates": (1.35, 103.82),
    },
    "Johannesburg": {
        "population": 957441,
        "category": "want to visit",
        "fact": "The Soweto township of Johannesburg was once home to Nelson Mendela.",
        "coordinates": (-26.20, 28.04),
    },
    "Lima": {
        "population": 268352,
        "category": "have visited",
        "fact": "Places associated with Lima were once known as Limaq, named by Peruvian natives.",
        "coordinates": (-12.04, -77.04),
    },
    "Arviat": {
        "population": 2657,
        "category": "want to visit",
        "fact": "Arviat, once known as Eskimo Point, is home to different forcibly relocated Inuit groups.",
        "coordinates": (61.10, -94.06),
    },
}
...
if (
    city in cd_list
):  # IF USER SELECTED CITY IS IN OUR LIST OF CITIES, MAKE THEIR SELECTED CITY THE OUTPUT CITY
    city_out = city
else:
    print(
        "Sorry, for the Beta to work you must pick one of the following cities:",
        city_list,
    )  # OTHERWISE FAIL
    exit()
...
print(
    "City Name:\t{0}\nPopulation:\t{1}\nInteresting Fact:\t{2}\nLatitude:\t{3}\nLongitude:\t {4}".format(
        name, population, fact, latitude, longitude
    )
)
...
for city in city_dict:
    for compcity in city_dict:
        multiplier = float(city_dict[city]["population"]) / (
            city_dict[compcity]["population"]
        )
